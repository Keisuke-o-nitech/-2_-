...
スケッチの概要
 ...
 ※これに加えてPID係数，目標位置の調整のため，BluetoothSerial経由でデータを出力できるようにした．※
*/

// M5系列の製品共通のライブラリ
#include <M5Unified.h>
// BluetoothSerialを行うライブラリ
#include "BluetoothSerial.h"

//SerialBTという名前でインスタンス化
...
void setup(){
  ...
  //Serial通信のボーレートを9600に指定
  Serial.begin(9600);
  //BluetoothSerialのデバイス名を"M5StickCPlusSerial"に指定
  SerialBT.begin("M5StickCPlusSerial");
...
void loop() {
  ...
  */
  ...
  //Serial出力する文字列を格納する変数
  char buf[60];
  //bufに格納する文字列を作成
  //偏差now と モータの回転量powerを出力させる
  sprintf(buf, "%6.5f,%6.5f\n", now,power);
  //BluetoothSerialで文字列を送信
  SerialBT.print(buf);
}